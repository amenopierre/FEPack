...
numNodesK = 24;
meshK = meshes.MeshRectangle(1, Khs(1)+[-0.1, 0.1], Khs(2)+[-0.1, 0.1], numNodesK, numNodesK);

meshK.MeshRectangleLocallyRefined(0, Khs(1)+[-0.1, 0.1], Khs(2)+[-0.1, 0.1], numNodesK, numNodesK, Khs, 1e-4);
...
%
% fid = fopen(['outputs/disp_surf.txt'], 'w+');
% % Kdom = [];
...
fid = fopen('outputs/dispersion_patch_near_K.txt', 'w+');
fprintf(fid, 'pt1\t pt2\t pt3\n');
fprintf(fid, '%0.6d\t %0.6d\t %0.6d\n', -1+meshK.triangles(:, 1:3).');
fclose(fid);

fid = fopen('outputs/dispersion_surfaces_near_K.txt', 'w+');
fprintf(fid, 'x\t y');
for idJ = 1:numEVP
