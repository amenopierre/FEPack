...
numNodesK = 4;
numNodesK1 = ceil((4*pi/3) * numNodesK);
numNodesK2 = ceil((2*pi/sqrt(3)) * numNodesK);
meshK = meshes.MeshRectangle(1, [-(k1(1)+0.5) (k1(1)+0.5)], [-(Khs(2)+0.5) (Khs(2)+0.5)], numNodesK1, numNodesK2);
meshK.MeshRectangleLocallyRefined(0, [-(k1(1)+0.5) (k1(1)+0.5)], [-(Khs(2)+0.5) (Khs(2)+0.5)], 64, 64, brillouinVerts, 0.01*ones(1, 6));
% meshK = meshes.MeshRectangle(1, [0 +k1(1)], [0 +Khs(2)], numNodesK1, numNodesK2);
% meshK = meshes.MeshRectangle(1, [-k1(1) 0], [0 +Khs(2)], numNodesK1, numNodesK2);
% meshK = meshes.MeshRectangle(1, [0 +k1(1)], [-Khs(2) 0], numNodesK1, numNodesK2);
% meshK = meshes.MeshRectangle(1, [-k1(1) 0], [-Khs(2) 0], numNodesK1, numNodesK2);
% EtoK = [k1, k2];
% Kpts = (EtoK * meshK.points(:, 1:2).').';
...
%%
fid = fopen('outputs/D0_Kdomaine.txt', 'w+');
fprintf(fid, 'pt1\t pt2\t pt3\n');
fprintf(fid, '%0.6d\t %0.6d\t %0.6d\n', -1+meshK.triangles.');
fclose(fid);

fid = fopen('outputs/D0_dispersion_surfaces.txt', 'w+');
fprintf(fid, 'x\t y');
for idJ = 1:numEVP
